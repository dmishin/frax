...
public class UniversalRendererContext extends RenderingContext {
	 ...
	 */
	private ExprNode parse(String ipFormula) throws ScriptSyntaxException{
		//is constant?
	...
	private ExprNode[] parseChildrenTokens(String[] tokens) throws ScriptSyntaxException {
	...
	private String[] splitString(String ipFormula) throws ScriptSyntaxException {
		...
		int i=0;
		for (Iterator iToken = tokens.iterator(); iToken.hasNext();++i) {
			rval[i] = (String) iToken.next();
		}
	...
	private UnaryNode makeFuncNode(String funcname, ExprNode child) throws ScriptSyntaxException{
	...
	private VarNode getVariable(String name) {
		VarNode var = (VarNode )vars.get(name);
		if (var==null){
	...
	public UniversalRendererContext(){
	...
	public void setFormula(String formula) throws ScriptSyntaxException{
		...
		vars.put(varY.getName(), varY);
		vars.put(varIter.getName(), varIter);

		ExprNode nd= parse(formula);
		...
		if (finish==null) finish=getVariable("I");
